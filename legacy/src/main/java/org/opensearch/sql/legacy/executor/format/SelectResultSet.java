...
public class SelectResultSet extends ResultSet {
   ...
   * object the key and the Object it refers to the value. This handles the case of regular object
   * since nested objects will not be in hit.source but rather in hit.innerHits
   *
  ...
  /**
   * If innerHits associated with column name exists, flatten both the inner field name and the
   * inner rows in it.
   *
   * <p>Sample input: newKeys = {'region', 'employees.age'}, row = {'region': 'US'} innerHits =
   * employees: { hits: [{ source: { age: 26, firstname: 'Hank' } },{ source: { age: 30, firstname:
   * 'John' } }] }
   */
  ...
  /**
   * Do Cartesian Product between current outer row and inner rows by nested loop and remove
   * original outer row.
   *
   * <p>Sample input: colName = 'employees', rows = [{region: 'US'}] colValue= [{ source: { age: 26,
   * firstname: 'Hank' } },{ source: { age: 30, firstname: 'John' } }]
   *
   * <p>Return: [ {region:'US', employees.age:26, employees.firstname:'Hank'}, {region:'US',
   * employees.age:30, employees.firstname:'John'} ]
   */
